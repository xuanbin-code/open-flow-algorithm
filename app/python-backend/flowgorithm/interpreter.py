...
def evaluate_expression(
    ...
    if "&" in expr:
      ...
      if len(parts) > 1:
          return "".join(
              format_output(evaluate_expression(part, variables, functions))
              for part in parts
              if part.strip()
          )
...
def _split_concat_expression(expr: str) -> list[str]:
    """Split Flowgorithm string concatenation on top-level '&' operators."""
    parts: list[str] = []
    current: list[str] = []
    depth = 0
    quote: str | None = None
...
class FlowgorithmInterpreter:
    ...
    def _execute_user_function(self, func_name: str, args: list[Any]) -> Any:
        ...
        while self._current_index < len(self._current_body):
            ...
            # Keep call/visualization events generated by nested sync execution.
